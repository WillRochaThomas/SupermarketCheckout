...
3. Each test corresponds to an application feature. If a test passes, it's fair to assume the feature is complete.
   ...
   A scenario equates to one test. Each scenario may have a number of steps to complete, as part of the test.
   In this example, the feature's test has three steps: one passed, the next step was marked as 'pending' (not implemented yet) and as a result the last step was skipped.
   This means just enough functionality to pass the first step has been implemented, and the bulk of the feature is still not implemented.
