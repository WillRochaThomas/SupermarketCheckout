...
3. Each test corresponds to an application feature. If a test passes, it's fair to assume the feature is complete.
   ...
   In this example, the feature's test has three steps (given, when, then) and only one passed.
   This means just enough functionality to setup the test's preconditions has been implemented (the 'Given' step passes).
   The bulk of the feature is still pending.
