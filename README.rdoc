...
1. At the command prompt, clone this repository:
       <tt>git clone https://github.com/WillJThomas/SupermarketCheckout</tt>

2. Change directory to <tt>SupermarketCheckout</tt> and start the web server:
       <tt>cd SupermarketCheckout; rails server</tt> (run with --help for options)
...
features
  The behavioural tests, which were used to drive the development (BDD). Cucumber was used to write these tests.
...
script
  Helper scripts for automation and generation e.g. cucumber scripts for generating new behavioural tests.
